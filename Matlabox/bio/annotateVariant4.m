function annotateVariant4(variantFn, cohortReadCountFn, sampleReadCountFn, rnaAlleleCountFn, ...
    GTExSampleFn, sampleGTExGTFReadCountFn, outputFn, varargin)
    ...
    para.GTEx = '/nethome/bjchen/DATA/GTEx/20140117/GTEx_Gene.mat'; %can pre-load to speed up multiple samples    
    ...
    para.gencode = '/nethome/bjchen/DATA/GenomicInfo/GENCODE/GENCODE.gene.v18.mat';
    ...
    %get GTEx expression
    [gtexDataHeader, gtexData] = getGtexData(GTExSampleFn, sampleGTExGTFReadCountFn, variant, para);
...
function [expDataHeader, expData] = getExpression(cohortReadCountFn, sample, variant, para)
    ...
    GENCODE = loadStructData(para.gencode);
    %read expression
    if ischar(cohortReadCountFn)        
        if ~isempty(strfind(cohortReadCountFn, '.mat'))
            cohort = loadStructData(cohortReadCountFn);
        else
            cohort = parseText(cohortReadCountFn, 'nrowname', 2, 'ncolname', 1, 'numeric', true);
            ...
            cohort.rowname(:, rmi) = [];
            cohort.featureCounts = cohort.text;
            cohort.geneId = cohort.rowname;
            cohort.sample = cohort.colname;
            cohort = rmfield(cohort, {'rowname', 'colname', 'text', 'rownamelabel'});
            cohort.DESeq_normalized = RNACOUNT.getDESeqNormalizedCount(cohort.featureCounts);
            cohort = DATASTRUCTFUNC.orderDataStruct(cohort, 'geneId', sample.geneId);
        end
    else
    ...
    sample.geneName = GENOMEFUNC.idQuery(GENCODE, sample.geneId, 'name', 'id');    
    if any(~strcmp(sample.geneId, cohort.geneId))
...
function [gtexDataHeader, gtexData] = getGtexData(GTExSampleFn, sampleGTExGTFReadCountFn, variant, para)
    if ~isempty(para.GTEx) && ~isempty(sampleGTExGTFReadCountFn)
        nvar = length(variant.locidx);
        if ischar(para.GTEx)
        ...
        f = fopen(GTExSampleFn);
        GTExSample = textscan(f, '%s');
        GTExSample = GTExSample{1};
        fclose(f);                
        
        %limit to subset of samples 
        GTEx = DATASTRUCTFUNC.orderDataStruct(GTEx, 'sample', GTExSample);
        GTEx.pctSampleExpressed = NaN(length(GTEx.gene), 3);
        
        sample = RNASTATS.readFeatureCountSingleFile(sampleGTExGTFReadCountFn);

        [~, sampGeneIdx, gtexGeneIdx] = GENOMEFUNC.intersectGencode(sample.geneId, GTEx.gencodeId);
        gtexLogRead = log2( RNACOUNT.getDESeqNormalizedCount( GTEx.readcount(gtexGeneIdx, :))+1);
        gtexLogMedian = nanmedian(gtexLogRead , 2);
        
        sampleLogCount = log2(RNACOUNT.getDESeqNormalizedCount(GTEx.readcount(gtexGeneIdx,:), sample.count(sampGeneIdx,:))+1);
        ...
        for i = 1:3
            GTEx.pctSampleExpressed(:,i) = sum(GTEx.discreteExpLevel==i,2) ./ length(GTEx.sample);
        end
