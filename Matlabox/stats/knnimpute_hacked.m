function imputed = knnimpute_hacked(data,K,varargin)
...
if strcmp(metric, 'nancorr')
    noNans = true(size(data,1),1);
else
...
if strcmp(metric, 'nancorr')
    distances = pdist(dataNoNans', @(xi,xj) 1-corr(xj',xi','rows','pairwise')); 
else
