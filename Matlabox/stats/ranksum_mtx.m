function [z, p, sumofrank] = ranksum_mtx(x,y,varargin)
...
switch technique
	case 'full_enumeration'
        ...
        np = size(allpos, 1);
        allpos = ranks( allpos, :);
        sumranks = zeros(np, ntest);
        for posidx = 1:np
            sumranks(posidx, :) = sum( allpos(posidx:np:end,:), 1);
        end
...
% 		np = size(sumranks, 1);
		...
		switch tail
			case 'both'
                plo = sum(bsxfun(@le, sumranks, w), 1) ./ np; %row vec
                phi = sum(bsxfun(@ge, sumranks, w), 1) ./ np;
% 				plo = sum( sumranks <= w) / np ;
% 				phi = sum( sumranks >= w) / np ;
				p_tail = min(plo,phi); %row vec
				p = min(2*p_tail, 1);   % 2-sided, p>1 means middle is double-counted
				
			case 'right'
				switch same_order
					case true
                        p = sum(bsxfun(@ge, sumranks, w), 1) ./ np;
% 						p = sum( sumranks >= w) / np ;
					case false
                        p = sum(bsxfun(@le, sumranks, w), 1) ./ np; %row vec
% 						p = sum( sumranks <= w) / np;
				end
...
% 		end
	...
	case 'normal_approximation'
		wmean = ns*(nx + ny + 1)/2;
		tiescor = 2 .* tieadj ./ ((nx+ny) * (nx+ny-1)); %1 x ntest
		wvar  = nx*ny*((nx + ny + 1) - tiescor)./12; %1 x ntest
		wc = w - wmean; %row vec
...
if nargout > 2,
    if same_order
        ...
        sumofrank = sum( ranks(ns+1:end, :), 1);
    end
end
