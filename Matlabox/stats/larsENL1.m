function [larsenmodel] = larsENL1(X, y, lambda2, stop, trace, L1norms, maxstepnum)
...
%   bj: revised 11/13/07
%   add L1norms to train models with multiple L1norm constraints
%

    %bj: including data transformation!!!!!
    larsenmodel.meanx = nanmean(X);
    larsenmodel.ybar = nanmean(y);
    larsenmodel.normx = zeros(1,size(X,2));
    scx = X - repmat(larsenmodel.meanx,size(X,1),1);
    scy = y - larsenmodel.ybar;
    for i = 1:size(scx,2)
        larsenmodel.normx(i) = norm(scx(:,i));
        scx(:,i) = scx(:,i) / larsenmodel.normx(i);
    end
    ...
    if nargin < 6
        fprintf('invalid usage; should specify L1norms.\n');
        return
    end

    %% Elastic net variable setups
    [n p] = size(scx);
    maxk = 10*(n+p); % Maximum number of iterations
    maxk = min(maxk, maxstepnum);
...
%     %bj: force #max selected features <= n-1
%     %don't need that many
%     if n<p
%         nvars = min(n-1,p)+10; %+10 for extra
%     else
%         nvars = p;
%     end
    
    %bj: fill in the first L1norm if existed

    L1ni = 1;
    stop = L1norms(1);
    larsenmodel.beta = zeros(p, length(L1norms));
    
    %from original code...why?
    ...
    while vars < nvars && ~stopcond && k < maxk
        ...
        if ~lassocond % if a variable has been dropped, do one iteration with this configuration (don't add new one right away)
            R = cholinsert(R,scx(:,j),scx(:,A),lambda2);
            A = [A j];
            I(I == j) = [];
            vars = vars + 1;
            if trace
        ...
        AA = 1/sqrt(sum(GA1.*s));
        w = AA*GA1;
        u1 = scx(:,A)*w*d2; % equiangular direction (unit vector) part 1
        u2 = zeros(p, 1); u2(A) = d1*d2*w; % part 2
        if vars == nvars % if all variables active, go all the way to the lsq solution
            gamma = C/AA;
        else
            a = (scx'*u1 + d1*u2)*d2; % correlation between each variable and eqiangular vector
            temp = [(C - c(I))./(AA - a(I)); (C + c(I))./(AA + a(I))];
            gamma = nanmin([temp(temp > 0); C/AA]);
        end

        % LASSO modification
        lassocond = 0;
        temp = -beta(k,A)./w';
        [gamma_tilde] = nanmin([temp(temp > 0) gamma]);  
        ...
        if stop > 0
            ...
            if t2 >= stop
                ...
                %bj
                larsenmodel.beta(:,L1ni) = (beta(k,:) + s*(beta(k+1,:) - beta(k,:))) / d2;
                while L1ni+1 <= length(L1norms)
                    L1ni = L1ni + 1;
                    stop = L1norms(L1ni) / sqrt(1+lambda2);
                    s = (stop - t1)/(t2 - t1);
                    if t2 >= stop
                        larsenmodel.beta(:,L1ni) = (beta(k,:) + s*(beta(k+1,:) - beta(k,:))) / d2;
                        %larsenmodel.beta(L1ni,:) = larsenmodel.beta(L1ni-1,:);
                    else
                        break
                    end                    
                end
                if L1ni == length(L1norms) && t2 >= stop %stop has changed
                    stopcond = 1;
                end
            end
        end
    ...
    %save space
    larsenmodel.beta = sparse(larsenmodel.beta);
    if k == maxk && k ~= maxstepnum
